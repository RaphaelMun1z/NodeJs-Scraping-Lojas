...
export const configuracaoMatching = {
	...
	dimensaoEmbedding: Number(process.env.EMBEDDING_DIMENSIONS || 768),
	embeddingUrl: process.env.EMBEDDING_URL?.trim() || undefined,
	embeddingModelo: process.env.EMBEDDING_MODEL?.trim() || "local-hash-v1",
...
export function validarConfiguracaoMatching(): void {
	...
	if (configuracaoMatching.embeddingUrl && !configuracaoMatching.embeddingModelo) throw new Error("EMBEDDING_MODEL é obrigatório quando EMBEDDING_URL está configurado");
