...
function resumirTituloOferta(titulo, limite = 125) { const texto = String(titulo ?? "").trim(); if (texto.length <= limite) return texto; return `${texto.slice(0, limite).replace(/\s+\S*$/, "")}…`; }
...
function configurarPainelMonitoramento() {
	...
	$("#close-monitor-logs").addEventListener("click", () => { dadosMonitoramento.execucaoAbertaId = ""; $("#monitor-logs-modal").classList.add("is-hidden"); });
	$("[data-close-monitor-logs]").addEventListener("click", () => { dadosMonitoramento.execucaoAbertaId = ""; $("#monitor-logs-modal").classList.add("is-hidden"); });
...
function formatarTempoRestante(milisegundos) { if (!Number.isFinite(milisegundos) || milisegundos <= 0) return ""; const segundos = Math.max(1, Math.round(milisegundos / 1000)); if (segundos < 60) return `~${segundos}s restantes`; const minutos = Math.round(segundos / 60); if (minutos < 60) return `~${minutos}min restantes`; const horas = Math.floor(minutos / 60); const minutosRestantes = minutos % 60; return `~${horas}h${minutosRestantes ? ` ${minutosRestantes}min` : ""} restantes`; }
function estimarTempoRestante(valor, iniciadoEm) { const percentual = Number(valor); const inicio = iniciadoEm ? new Date(iniciadoEm).getTime() : 0; const decorrido = Date.now() - inicio; if (percentual >= 100) return "Concluída"; if (!inicio || percentual <= 0 || decorrido < 1000) return "Calculando…"; return formatarTempoRestante((decorrido / percentual) * (100 - percentual)) || "Calculando…"; }
function renderizarBarraProgresso(valor, rotulo = "", tempoRestante = "") { const percentual = Math.max(0, Math.min(100, Number(valor) || 0)); const classeEstado = percentual >= 100 ? " is-complete" : " is-active"; const barraGeral = rotulo === "Progresso geral"; const eta = tempoRestante ? `<span class="progress-eta">${tempoRestante}</span>` : ""; return `<div class="progress-item${barraGeral ? " progress-item-general" : ""}">${barraGeral ? `<div class="progress-label progress-label-general"><span><small>SEU PROGRESSO</small><strong>${percentual}% <em>${percentual >= 100 ? "concluído" : "concluído"}</em></strong></span><span class="progress-remaining">${tempoRestante || "Calculando…"}</span></div>` : `<div class="progress-label"><span>${rotulo}</span><strong>${percentual}%</strong>${eta}</div>`}<div class="progress-track"><span class="progress-fill${classeEstado}" style="width: ${percentual}%"></span></div>${barraGeral ? '<span class="progress-stage">Acompanhando o processo de busca de produtos</span>' : ""}</div>`; }
function renderizarProgressoExecucao(progresso = {}, incluirGeral = false, iniciadoEm) { const barras = [renderizarBarraProgresso(progresso.coleta, "Coleta", estimarTempoRestante(progresso.coleta, iniciadoEm)), renderizarBarraProgresso(progresso.classificacao, "Classificação", estimarTempoRestante(progresso.classificacao, iniciadoEm)), renderizarBarraProgresso(progresso.embeddings, "Embeddings", estimarTempoRestante(progresso.embeddings, iniciadoEm)), renderizarBarraProgresso(progresso.indexacao, "Indexação", estimarTempoRestante(progresso.indexacao, iniciadoEm))]; if (incluirGeral) barras.unshift(renderizarBarraProgresso(progresso.geral, "Progresso geral", estimarTempoRestante(progresso.geral, iniciadoEm))); return barras.join(""); }
function formatarStatusHistorico(status, execucao = {}) { if (status === "executando" && !execucao.erro && !execucao.finalizadoEm) return `<div class="history-status-progress"><span class="history-status status-running"><span class="status-icon status-loading"></span>Executando</span>${renderizarBarraProgresso(execucao.progresso?.geral, "", estimarTempoRestante(execucao.progresso?.geral, execucao.iniciadoEm))}</div>`; if (status === "erro" || execucao.erro) return '<span class="history-status status-error"><span class="status-icon">×</span>Erro</span>'; return '<span class="history-status status-success"><span class="status-icon">✓</span>Concluído</span>'; }
...
let dadosMonitoramento = { execucoes: [], statusFontes: [], logs: [], resumo: {}, execucaoAbertaId: "", execucaoAberta: null, paginaHistorico: 1, totalPaginasHistorico: 0, filtrosHistorico: { fonte: "", dataInicio: "", dataFim: "" } };
...
function renderizarPainelExecucaoAberta(execucao) { const painel = $("#monitor-execution-progress"); if (!painel || !execucao) return; if (execucao.status === "erro" || execucao.erro) { painel.innerHTML = `<div class="monitor-execution-failed"><span class="monitor-execution-failed-icon" aria-hidden="true">!</span><div><strong>Execução interrompida</strong><p>${escaparHtml(execucao.erro ?? "A busca não foi concluída. Tente novamente.")}</p></div></div>`; return; } painel.innerHTML = renderizarProgressoExecucao(execucao.progresso, true, execucao.iniciadoEm); }
function atualizarPainelExecucaoAberta(execucao) { if (!dadosMonitoramento.execucaoAbertaId || !execucao?._id || String(execucao._id) !== dadosMonitoramento.execucaoAbertaId) return; dadosMonitoramento.execucaoAberta = { ...dadosMonitoramento.execucaoAberta, ...execucao }; renderizarPainelExecucaoAberta(dadosMonitoramento.execucaoAberta); }
function renderizarLogsMonitoramento() { const area = $("#monitor-logs"); if (!area) return; const estavaNoFinal = area.scrollHeight - area.scrollTop - area.clientHeight < 24; const filtroFonte = $("#monitor-source-filter")?.value; const niveisSelecionados = [...document.querySelectorAll("input[name='monitor-log-level']:checked")].map((caixa) => caixa.value); area.innerHTML = dadosMonitoramento.logs.filter((item) => (!dadosMonitoramento.execucaoAbertaId || String(item.execucaoId) === dadosMonitoramento.execucaoAbertaId) && (!filtroFonte || item.fonte === filtroFonte) && niveisSelecionados.includes(item.nivel)).map((item) => `<div class="monitor-log ${item.nivel}"><time>${formatarHorario(item.criadoEm)}</time><strong>${renderizarFonteComLogo(item.fonte)}</strong><span>${escaparHtml(item.mensagem)}</span></div>`).join("") || '<div class="empty-state">Nenhuma mensagem registrada para esta execução.</div>'; if (estavaNoFinal) area.scrollTop = area.scrollHeight; }
...
async function carregarLogsExecucao(id) { dadosMonitoramento.execucaoAbertaId = String(id); dadosMonitoramento.execucaoAberta = null; $("#monitor-execution-progress").innerHTML = '<div class="monitor-execution-loading">Carregando detalhes desta execução…</div>'; $("#monitor-logs").innerHTML = '<div class="empty-state">Carregando logs desta execução…</div>'; const [respostaExecucao, respostaLogs] = await Promise.all([fetch(`/api/admin/scraping/execucoes/${encodeURIComponent(id)}`), fetch(`/api/admin/scraping/execucoes/${encodeURIComponent(id)}/logs`)]); if (!respostaExecucao.ok || !respostaLogs.ok || dadosMonitoramento.execucaoAbertaId !== String(id)) return; const execucao = (await respostaExecucao.json()).dados; dadosMonitoramento.execucaoAberta = execucao; dadosMonitoramento.logs = (await respostaLogs.json()).dados ?? []; renderizarPainelExecucaoAberta(execucao); $("#monitor-logs-modal").classList.remove("is-hidden"); renderizarLogsMonitoramento(); $("#monitor-logs").scrollTop = $("#monitor-logs").scrollHeight; }
function adicionarEventoMonitoramento(evento) { if (evento.tipo === "log") { dadosMonitoramento.logs.push(evento.dados); if (dadosMonitoramento.logs.length > 500) dadosMonitoramento.logs.shift(); renderizarLogsMonitoramento(); } if (evento.tipo === "execucao") { const indice = dadosMonitoramento.execucoes.findIndex((item) => item._id === evento.dados._id); if (indice >= 0) dadosMonitoramento.execucoes[indice] = { ...dadosMonitoramento.execucoes[indice], ...evento.dados }; else dadosMonitoramento.execucoes.unshift(evento.dados); atualizarPainelExecucaoAberta(evento.dados); renderizarStatusMonitoramento(); renderizarHistoricoMonitoramento(); } }
