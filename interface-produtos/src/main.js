...
const estado = { pagina: 1, limite: 20, totalPaginas: 0, itens: [], busca: "", categoria: "", fonte: "", precoMin: "", precoMax: "", apenasAtivos: true, ordenacao: "recent" };
...
async function carregarCategorias() {
	...
	try {
		...
		area.innerHTML = `<label class="radio-row category-all-row"><input type="radio" name="category" value="" ${estado.categoria ? "" : "checked"} /> Todas as categorias</label>${[...grupos.entries()].map(([principal, subcategorias]) => `<div class="category-filter-group"><div class="category-filter-heading"><label class="radio-row"><input type="radio" name="category" value="${escaparHtml(principal)}" ${estado.categoria === principal ? "checked" : ""} /> <strong>${escaparHtml(principal)}</strong></label></div>${subcategorias.length ? `<div class="category-subcategories">${subcategorias.map((subcategoria) => { const valor = `${principal} > ${subcategoria}`; return `<label class="radio-row"><input type="radio" name="category" value="${escaparHtml(valor)}" ${estado.categoria === valor ? "checked" : ""} /> <span>${escaparHtml(subcategoria)}</span></label>`; }).join("")}</div>` : ""}</div>`).join("")}`;
