...
const estado = { pagina: 1, limite: 20, totalPaginas: 0, itens: [], busca: "", fonte: "", precoMin: "", precoMax: "", apenasAtivos: true, ordenacao: "recent" };
...
function renderizarPrecos(item) {
	...
	return `<div class="product-prices">${item.precoAntigo > item.preco ? `<span class="old-price">${formatarPreco(item.precoAntigo)}</span>` : ""}<div class="current-price-row"><strong class="product-price">${formatarPreco(item.preco)}</strong>${desconto ? `<span class="discount-badge">-${desconto}%</span>` : ""}</div></div>`;
}
...
function renderizarNovidades() {
	...
	gradeNovidades.innerHTML = estadoNovidades.itens.slice(inicio, inicio + estadoNovidades.porPagina).map(renderizarCardNovidade).join("");
...
function renderizarProdutos() {
	...
	if (estado.ordenacao === "price-asc") itens.sort((a, b) => (a.preco ?? Infinity) - (b.preco ?? Infinity));
	if (estado.ordenacao === "price-desc") itens.sort((a, b) => (b.preco ?? 0) - (a.preco ?? 0));
...
async function carregarProdutos() {
	...
	if (estado.busca) parametros.set("busca", estado.busca); if (estado.fonte) parametros.set("fonte", estado.fonte); if (estado.precoMin) parametros.set("precoMin", estado.precoMin); if (estado.precoMax) parametros.set("precoMax", estado.precoMax); if (estado.apenasAtivos) parametros.set("ativo", "true");
...
function prepararOfertasDetalhe(detalhe, ofertas) {
	if (!Array.isArray(ofertas) || ofertas.length < 2) return;
	const observador = new MutationObserver(() => {
		...
		secao.innerHTML = `<div class="history-heading"><div><h2>Ofertas nas lojas</h2><span>${ofertas.length} loja(s) encontradas</span></div></div><div class="offers-list">${ofertas.map((oferta) => `<div class="offer-row"><span>${renderizarFonteComLogo(oferta.fonte)}</span><strong>${formatarPreco(oferta.preco)}</strong>${oferta.ativo === false ? "<span>Inativo</span>" : ""}${oferta.url ? `<a class="store-link" href="${escaparHtml(oferta.url)}" target="_blank" rel="noreferrer">Ver oferta ↗</a>` : ""}</div>`).join("")}</div>`;
...
function renderizarResumoMonitoramentoLegado() { const resumo = dadosMonitoramento.resumo; $("#monitor-summary").innerHTML = `<div class="monitor-stat"><strong>${resumo.ultimaAtualizacao ? formatarDataHora(resumo.ultimaAtualizacao) : "—"}</strong><span>Última atualização</span></div><div class="monitor-stat"><strong>${resumo.proximaBusca ? formatarDataHora(resumo.proximaBusca) : "—"}</strong><span>Próxima busca</span></div><div class="monitor-stat"><strong>${resumo.produtosAtivos ?? 0}</strong><span>Produtos listados atualmente</span></div><div class="monitor-stat"><strong>${resumo.duracaoMediaMs ? formatarDuracao(resumo.duracaoMediaMs) : "—"}</strong><span>Tempo médio das buscas</span></div>`; }
...
async function carregarLogsExecucao(id) { const resposta = await fetch(`/api/admin/scraping/execucoes/${encodeURIComponent(id)}/logs`); if (!resposta.ok) return; dadosMonitoramento.logs = (await resposta.json()).dados ?? []; $("#monitor-logs-modal").classList.remove("is-hidden"); renderizarLogsMonitoramento(); $("#monitor-logs").scrollTop = $("#monitor-logs").scrollHeight; }
...
function renderizarResumoMonitoramento() { const resumo = dadosMonitoramento.resumo; clearInterval(temporizadorContagemRegressiva); $("#monitor-summary").innerHTML = `<div class="monitor-stat">${formatarDataResumo(resumo.ultimaAtualizacao)}<span>Última atualização</span></div><div class="monitor-stat"><strong class="monitor-stat-time">${resumo.proximaBusca ? new Date(resumo.proximaBusca).toLocaleTimeString("pt-BR") : "—"}</strong><small class="monitor-stat-date">${resumo.proximaBusca ? new Date(resumo.proximaBusca).toLocaleDateString("pt-BR") : ""}</small><span>Próxima busca</span></div><div class="monitor-stat"><strong>${resumo.produtosAtivos ?? 0}</strong><span>Produtos listados atualmente</span></div><div class="monitor-stat"><strong>${resumo.duracaoMediaMs ? formatarDuracao(resumo.duracaoMediaMs) : "—"}</strong><span>Tempo médio por rodada</span></div>`; atualizarContagemRegressiva(); temporizadorContagemRegressiva = setInterval(atualizarContagemRegressiva, 1000); }
...
$("#clear-filters").addEventListener("click", () => { estado.busca = ""; estado.fonte = ""; estado.precoMin = ""; estado.precoMax = ""; estado.apenasAtivos = true; $("#search-input").value = ""; $("input[name='source'][value='']").checked = true; $("#min-price").value = 0; $("#max-price").value = 20000; $("#only-active").checked = true; atualizarValoresPreco(); estado.pagina = 1; carregarProdutos(); });
$("#sort-filter").addEventListener("change", (evento) => { estado.ordenacao = evento.target.value; renderizarProdutos(); }); document.querySelectorAll("[data-category]").forEach((botao) => botao.addEventListener("click", () => { const estavaEmOutraPagina = !$("#admin-shell").classList.contains("is-hidden") || !$("#product-detail").classList.contains("is-hidden"); document.querySelectorAll("[data-category]").forEach((item) => item.classList.remove("is-active")); botao.classList.add("is-active"); estado.busca = botao.dataset.category ?? ""; $("#search-input").value = estado.busca; estado.pagina = 1; $("#admin-shell").classList.add("is-hidden"); $("#admin-page").classList.add("is-hidden"); $("#product-detail").classList.add("is-hidden"); $(".content-layout").classList.remove("is-hidden"); window.location.hash = ""; if (!estavaEmOutraPagina) carregarProdutos(); }));
configurarConta(); atualizarConta(); window.addEventListener("hashchange", renderizarRota); atualizarValoresPreco(); renderizarRota();
