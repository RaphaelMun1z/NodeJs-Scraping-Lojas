...
function definirCookie(resposta: Response, nome: string, valor: string, seguro: boolean): void {
	resposta.append("Set-Cookie", `${nome}=${encodeURIComponent(valor)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${DURACAO_SESSAO_MS / 1000}${seguro ? "; Secure" : ""}`);
}
...
export class ServicoAutenticacao {
	...
	async autenticar(email: string, senha: string, codigoTotp?: string): Promise<{ administrador: AdministradorAutenticado; token: string; tokenCsrf: string } | null> {
		...
		await ModeloSessaoAdministrador.create({ administradorId: administrador._id, tokenHash: gerarHash(token), tokenCsrfHash: gerarHash(tokenCsrf), expiraEm: new Date(Date.now() + DURACAO_SESSAO_MS), criadaEm: new Date() });
	...
	async obterAdministrador(requisicao: Request): Promise<{ administrador: AdministradorAutenticado; tokenCsrfHash: string } | null> {
		const token = lerCookies(requisicao)[NOME_COOKIE_SESSAO];
		...
		return { administrador: { id: administrador._id.toString(), email: administrador.email, papel: administrador.papel, mfaAtivo: administrador.mfaAtivo }, tokenCsrfHash: sessao.tokenCsrfHash };
	}

	definirCookies(resposta: Response, token: string, tokenCsrf: string): void {
		definirCookie(resposta, NOME_COOKIE_SESSAO, token, this.cookieSeguro);
		resposta.append("Set-Cookie", `${NOME_COOKIE_CSRF}=${encodeURIComponent(tokenCsrf)}; Path=/; SameSite=Strict; Max-Age=${DURACAO_SESSAO_MS / 1000}${this.cookieSeguro ? "; Secure" : ""}`);
	}
