...
export class ServicoMatchingProduto {
	...
	async encontrarEquivalente(titulo: string, chaveIgnorada?: string, embeddingPronto?: number[]): Promise<ResultadoMatchingProduto> {
		...
		const candidatos = await this.indice.buscarCandidatos(tituloNormalizado, embedding, this.configuracao.candidatos, chaveIgnorada);
